...
const CategoryForm: React.FC<{ mode?: CategoryFormMode }> = ({ mode }) => {
  ...
  return (
    <div className="max-w-xl mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">{isEdit ? 'Edit Category' : 'Add Category'}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4" encType="multipart/form-data">
        ...
        <button type="submit" className="bg-green-700 text-white px-6 py-2 rounded font-semibold hover:bg-green-800 transition" disabled={loading}>{loading ? 'Saving...' : isEdit ? 'Update Category' : 'Add Category'}</button>
      </form>
    </div>
  );
};
